...
int main(int argc, char** argv) {
    if (argc != 8) {
        std::cout << "Usage: " << argv[0] 
                  << " <sources_json> <column_name> <save_path> <num_elements> <M> <ef_construction> <num_threads>" 
                  << std::endl;
    ...
    size_t num_elements = std::stoi(argv[4]);
    size_t M = std::stoi(argv[5]);
    size_t ef_construction = std::stoi(argv[6]);
    ...
    try {
        ...
        const size_t total_elements = data.GetRowCount();
        
        // Ensure num_elements doesn't exceed total available elements
        if (num_elements > total_elements) {
            std::cout << "Warning: Requested elements (" << num_elements 
                      << ") exceeds available elements (" << total_elements 
                      << "). Using all available elements." << std::endl;
            num_elements = total_elements;
        }
        
        // Create HNSW index with specified number of elements
        ...
        std::cout << "Successfully built index with parameters:" << std::endl
                  ...
                  << "- Elements indexed: " << num_elements << std::endl
                  << "- Total available elements: " << total_elements << std::endl
                  << "- M: " << M << std::endl
